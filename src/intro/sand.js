...
canvas.addEventListener("mousemove", (e) => {
  ...
  if (phase === "entranceReady") {
    const textWidth = Math.max(w * 0.25, 160);
    const textHeight = 60;
    const textX = w / 2 - textWidth / 2;
    const textY = h - 170; // centers around h - 140
    if (
      mouseX >= textX &&
...
function buildGirisText() {
  ...
  maskCanvas.width = w;
  maskCanvas.height = h;
  // Responsive font size calculation for 5 chars (enter)
  ...
  maskCtx.fillText("enter", w / 2, h * 0.48);
  const image = maskCtx.getImageData(0, 0, w, h).data;
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      const x = Math.floor(col * settings.cellSize + settings.cellSize / 2);
...
function updatePhase(dt) {
  ...
  if (phase === "hiddenHold") {
    ...
    if (phaseTime >= settings.hiddenFadeInSeconds) {
      phase = "entranceReady";
      phaseTime = 0;
    }
  }
...
function drawHiddenText() {
  ...
  ctx.save();
  const fontSize = Math.max(26, Math.min(36, w * 0.06));
  const lineText = "ENTER";
  ctx.globalAlpha = hiddenAlpha;
  ctx.fillStyle = isHovered ? "rgb(255, 232, 168)" : "rgb(236, 204, 116)";
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.font = `900 ${fontSize}px system-ui, -apple-system, sans-serif`;
  ctx.fillText(lineText, w / 2, h - 140);
  ctx.restore();
}
